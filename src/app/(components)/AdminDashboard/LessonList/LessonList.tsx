...
export default function LessonList() {
    const { confirmedLessons, isLoading, fetchConfirmedLessons } = useLessonContext();
    ...
    const getLessonSection = (dateStr: Date): string => {
        ...
        if (lessonDate.getTime() === tomorrow.getTime()) return "Tomorrow";
        if (lessonDate > today && lessonDate < nextWeek) return "This Week";
        return "Upcoming";
    };

    const groupLessonsBySection = () => {
        const grouped: Record<string, typeof confirmedLessons> = {
        "Today": [],
        "Tomorrow": [],
        "This Week": [],
        "Upcoming": [],
        ...
        const section = getLessonSection(new Date(lesson.date));
    ...
    const CustomMainComponent = () => {
        if (isLoading) return <p className="text-center py-4">Loading confirmed lessons...</p>;
        if (assignedLessons.length === 0) return <p className="text-center py-4">No confirmed lessons to display</p>;
